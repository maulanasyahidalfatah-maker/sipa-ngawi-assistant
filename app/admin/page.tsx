...
import { CheckCircle2, Clock, Upload, FileText, X, Eye } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // ✅ STATE BARU: Untuk Modal Lihat Bukti
      ...
      if (typeof window !== "undefined") {
        ...
        if (saved) {
          try {
            ...
            const formattedLocalTickets: AdminTicket[] = parsedData.map((item: any, idx: number) => {
              const num = idx + 3;
              const autoGeneratedId = `TK-0${num < 10 ? `0${num}` : num}`;
              const isValidCustomId = item.id && !["TK-001", "TK-002"].includes(item.id);

              return {
                id: isValidCustomId ? item.id : autoGeneratedId,
            ...
            setTickets([...formattedLocalTickets, ...defaultMockData]);
          } catch (e) {
  ...
  // 2. PROSES UNGGAH BUKTI PERBAIKAN & MENGUBAH STATUS TIKET
    ...
    setTimeout(() => {
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          alert("Ukuran file terlalu besar untuk disimpan di localStorage browser! Coba gunakan file/gambar berukuran lebih kecil (< 1MB).");
        }
      }
                    ...
                    <tr key={`${ticket.id}-${index}`} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-3.5 whitespace-nowrap">
                        {ticket.status === "PENDING" ? (
                          ...
                          /* ✅ PERBAIKAN: Tombol untuk membuka Modal Lihat Bukti secara langsung */
                          <button
                            type="button"
